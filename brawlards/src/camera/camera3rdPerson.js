...
export class ThirdPersonCameraController {
  ...
  _setupEventListeners() {
    ...
    this.renderer.domElement.addEventListener('wheel', this._onWheel.bind(this), { passive: false })
    this.renderer.domElement.addEventListener('contextmenu', this._onContextMenu.bind(this))
    window.addEventListener('resize', this._onResize.bind(this))
    
    // ✨ Reset camera moveState when focus changes (fix ghost camera movement)
    window.addEventListener('blur', () => this._resetMoveState());
   ...
   * Excludes cue - it was never dithered
   */
  _removeDithering() {
    if (!this.targetObject || !this.isDitheringActive) return
    ...
    // Traverse target object and restore original materials
    this.targetObject.traverse(child => {
      // Skip cue objects - they were never dithered
      if (isPartOfCue(child)) return
      ...
      materials.forEach((mat, index) => {
        ...
        if (this.originalMaterials.has(key)) {
          ...
          if (Array.isArray(child.material)) {
            child.material[index] = originalMat
          } else {
  ...
  dispose() {
    document.removeEventListener('pointerlockchange', this._onPointerLockChange.bind(this))
    document.removeEventListener('pointerlockerror', this._onPointerLockError.bind(this))
    ...
    this.renderer.domElement.removeEventListener('wheel', this._onWheel.bind(this))
    this.renderer.domElement.removeEventListener('contextmenu', this._onContextMenu.bind(this))
    ...
    this.ditherMaterials.forEach(mat => mat.dispose())
