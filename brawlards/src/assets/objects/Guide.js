...
function createGuide() {
  ...
  const stickGeometry = new THREE.CylinderGeometry(
    GUIDE_CONFIG.LIGHT_STICK_RADIUS,
    GUIDE_CONFIG.LIGHT_STICK_RADIUS,
    GUIDE_CONFIG.LIGHT_STICK_LENGTH,
    GUIDE_CONFIG.LIGHT_STICK_SEGMENTS
  )

  const lightStickMaterial = new THREE.MeshPhysicalMaterial({
    color: GUIDE_CONFIG.LIGHT_STICK_COLOR,
    emissive: GUIDE_CONFIG.LIGHT_STICK_EMISSIVE,
    emissiveIntensity: GUIDE_CONFIG.LIGHT_STICK_EMISSIVE_INTENSITY,
    roughness: 0.4,
    metalness: 0.1,
    transparent: true,
    opacity: GUIDE_CONFIG.LIGHT_STICK_OPACITY
  })

  const lightStick = new THREE.Mesh(stickGeometry, lightStickMaterial)
  ...
  lightStick.rotation.x = Math.PI / 2
  lightStick.position.set(0, stickY, stickZ)
  root.add(lightStick)

  // ----- Point Light for Light Stick -----
  const stickLight = new THREE.PointLight(
    GUIDE_CONFIG.POINT_LIGHT_COLOR,
    GUIDE_CONFIG.POINT_LIGHT_INTENSITY,
    GUIDE_CONFIG.POINT_LIGHT_DISTANCE
  )
  stickLight.position.copy(lightStick.position)
...
export function getGuideAsset() {
  return {
    name: "Guide",
    description: "Anh đưa em đi đou đó...?",
    factory: () => createGuide(),
