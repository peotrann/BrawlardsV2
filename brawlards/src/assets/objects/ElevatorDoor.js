...
function createElevatorDoor() {
  ...
  const updateDisplayTexture = function(ballCount, isCountdownActive) {
    ...
    // ✨ NEW: If countdown is active, always show green (15 will countdown to 0)
    if (isCountdownActive) {
      ...
      g = 255
      b = 0
    } else if (ballCount >= 10 && ballCount < 15) {
      // 10-14: Transition from red to green
      const ratio = (ballCount - 10) / 5
    ...
    const displayText = String(ballCount).padStart(2, '0')
