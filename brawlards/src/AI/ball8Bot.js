...
export class Ball8AI {
   ...
   */
  constructor(ballMesh, ballBody, scene, particleManager = null) {
    this.mesh = ballMesh
    this.body = ballBody
    this.scene = scene
    ...
    // ===== GENERIC ESCAPE MECHANISM (no hardcoded map dependencies) =====
    // Ball 8 uses raycast to detect escapes (holes, breaks, etc) below
    this.escapePointBelow = null  // Detected low point via raycast
    this.escapeSearchInterval = 0.3  // Search for escape points every 0.3s
    this.escapeSearchTimer = 0
    this.escapeDetectDistance = 5.0  // How far down to raycast search
    this.escapeAttractStrength = 0.4  // Strength to pull toward detected escape
    
    // Generic downward preference (works on any map)
    this.downwardBiasStrength = 0.08  // Subtle downward bias at normal (8%)
    this.panicDownwardAmplify = 4.0   // Panic amplifies downward bias 4x (becomes 32% at max panic)
  ...
  /**
   * Enhanced runtime escape point detection
   * Scans in multiple directions to find holes/weak points
   * Much more aggressive than simple straight-down raycast
   * Returns the best escape direction to move toward
   */
  detectEscapePointBelow() {
    this.escapeSearchTimer += this.fixedTimeStep
    
    if (this.escapeSearchTimer < this.escapeSearchInterval) {
      // Not yet time to search, use cached detection
      return this.escapePointBelow !== null
    }
    
    // Time to search for escape point below
    this.escapeSearchTimer = 0
    
    if (!this.scene) {
      this.escapePointBelow = null
      return false
    }
    
    // ===== AGGRESSIVE MULTI-DIRECTIONAL SCANNING =====
    // Scan in 9 directions: straight down + 8 diagonal/side directions
    const scanDirections = [
      ...
      new THREE.Vector3(1, -1, 1).normalize(),   // Down-right-forward
      new THREE.Vector3(-1, -1, 1).normalize(),  // Down-left-forward
      new THREE.Vector3(1, -1, -1).normalize(),  // Down-right-back
      new THREE.Vector3(-1, -1, -1).normalize()  // Down-left-back
    ]
    
    let bestEscapeDir = null
    let bestEscapeDistance = Infinity
    const escapeSearchRange = 8.0  // Increased from 5.0
    
    for (let scanDir of scanDirections) {
      const raycaster = new THREE.Raycaster(
        this.mesh.position.clone().add(scanDir.clone().multiplyScalar(0.5)),
        scanDir
      )
      ...
      // Strategy: Find gaps in geometry
      // No collision = complete gap/hole, or far intersection = likely hole
      
      if (intersections.length === 0) {
        // No collision - complete gap/hole
        bestEscapeDir = scanDir.clone()
        bestEscapeDistance = 0
        break  // Found direct hole, no need to search more
      } else if (intersections.length > 0) {
        // Check if intersection is far enough to indicate a gap
        const closestIntersection = intersections[0]
        
        // If closest hit is far, might be a hole
        if (closestIntersection.distance > 3.0) {
          if (closestIntersection.distance < bestEscapeDistance) {
            bestEscapeDir = scanDir.clone()
            bestEscapeDistance = closestIntersection.distance
          }
        }
      }
    }
    
    // ===== CONVERT DIRECTION TO ATTRACTION POINT =====
    if (bestEscapeDir) {
      // Project escape point ahead in the escape direction
      this.escapePointBelow = this.mesh.position.clone()
        .add(bestEscapeDir.clone().multiplyScalar(bestEscapeDistance + 2.0))
      return true
    }
   ...
   * Generic: adds downward bias instead of hardcoded hole locations
   */
  generateEvasionWaypoints(escapeDirection, playerPos) {
    ...
    // ===== GENERIC DOWNWARD BIAS (no hardcoded map dependencies) =====
    // Ball 8 always prefers moving downward (toward table edges/holes naturally)
    // Scaled by panic level: higher panic = stronger downward pull
    const panicDownwardBias = this.downwardBiasStrength * (1.0 + this.panicLevel * (this.panicDownwardAmplify - 1.0))
    const downwardDir = new THREE.Vector3(0, -1, 0)
    ...
    for (let i = 1; i <= waypointCount; i++) {
      ...
      // Add generic downward bias (works on any map)
      // Higher panic = stronger pull downward
      baseWaypoint.add(downwardDir.clone().multiplyScalar(panicDownwardBias * i * 0.5))
      
      // ===== DETECTED ESCAPE POINT ATTRACTION =====
      // If escape hole detected, blend waypoints toward it (scales with panic)
      if (this.escapePointBelow) {
        const dirToEscape = this.escapePointBelow.clone().sub(baseWaypoint).normalize()
        // Stronger attraction when panicking (up to 50% blend at max panic)
        const escapeAttractionStrength = 0.2 + this.panicLevel * 0.3  // 20% to 50%
        baseWaypoint.add(dirToEscape.clone().multiplyScalar(escapeAttractionStrength * 2.0))
      }
  ...
  updatePanicMode(delta, playerPos) {
    ...
    if (this.panicTimer <= 0) {
      ...
      // ===== AGGRESSIVE ESCAPE POINT SEEKING =====
      // At extreme panic (>0.8), actively seek detected escape points
      if (this.panicLevel > 0.8 && this.escapePointBelow) {
        const dirToEscape = this.escapePointBelow.clone()
          .sub(this.mesh.position)
          .normalize()
        // Very strong blend toward escape point (60-80% at extreme panic)
        const escapeBlendStrength = 0.6 + (this.panicLevel - 0.8) * 2.0  // 60% to 80%
        randomDir.lerp(dirToEscape, escapeBlendStrength)
      } else if (this.panicLevel > 0.85) {
        // No escape point detected, fallback to downward movement at extreme panic
        const downwardComponent = new THREE.Vector3(0, -1, 0)
        randomDir.lerp(downwardComponent, 0.3)  // 30% downward
      } else if (this.panicLevel > 0.5) {
        // Medium-high panic: add downward component
        const downwardComponent = new THREE.Vector3(0, -1, 0)
        randomDir.lerp(downwardComponent, 0.15)  // 15% downward
      }
  ...
  updateEvasionBehavior(delta, playerPos) {
    ...
    // Detect escape points below (generic, works on any map)
    this.detectEscapePointBelow()
